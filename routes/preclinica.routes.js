...
// Causa: el IMC se calcula en MySQL (columna generada a partir de
// PESO / (TALLA/100)^2). Si TALLA llega en 0 (o negativa) mientras
// PESO sí tiene valor, la división produce un resultado infinito/NaN
// que MySQL rechaza al insertar o actualizar.
// No usamos tieneInfoMinima() directamente aquí porque esa función
// exige peso Y talla > 0 simultáneamente, lo que rompería el guardado
// parcial de preclínica (flujo de "camposPendientes"). Esta validación
// solo bloquea el caso peligroso: un valor explícito <= 0 en peso o
// talla. Si ambos vienen vacíos, se deja pasar como dato pendiente.
// ------------------------------------------------------------
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // --- Fix 1: bloquear peso/talla <= 0 antes de tocar la BD ---
      const validacionIMC = validarPesoTallaParaIMC(peso, talla);
      if (!validacionIMC.valido) {
        return res.status(400).json({
          success: false,
          message: validacionIMC.mensaje,
        });
      }
      ...
      // --- Fix 3: rechazar solicitudes sin ningún dato clínico ---
      if (camposPendientes.length === CAMPOS_ESPERADOS_CONSULTA.length) {
        return res.status(400).json({
          success: false,
          message:
            "No se recibió ningún dato clínico para registrar la preclínica.",
          camposPendientes,
        });
      }
      ...
      // --- Fix 2: transacción explícita para INSERT + UPDATE tbl_citas ---
      ...
      if (connection) {
        try {
          ...
          console.error(
            "Error revirtiendo creación de preclínica:",
            rollbackError
          );
        }
      }
    ...
    try {
      ...
      if (camposPendientes.length === CAMPOS_ESPERADOS_CONSULTA.length) {
        return res.status(400).json({
          success: false,
          message:
            "No se recibió ningún dato clínico para actualizar la preclínica.",
          camposPendientes,
        });
      }
      ...
      // --- Fix 2: transacción explícita para UPDATE preclínica + cita ---
      connection = await pool.getConnection();
      await connection.beginTransaction();

      const [registroActual] = await connection.query(
        `
          SELECT ID_CITA, SIGNOS_VITALES_JSON
          FROM tbl_preclinica
          WHERE ID_PRECLINICA = ?
          LIMIT 1
          FOR UPDATE
      ...
      const signosJsonStr = JSON.stringify(
        construirSignosJson({
          signosVitalesJson:
            signosVitalesJson ?? registroActual[0].SIGNOS_VITALES_JSON,
          datos: datosClinicos,
          enviarAConsulta: debeEnviarAConsulta,
        })
      );
      ...
      if (connection) {
        try {
          ...
          console.error(
            "Error revirtiendo actualización de preclínica:",
            rollbackError
          );
        }
      }
    ...
    try {
      ...
      if (connection) {
        try {
          ...
          console.error(
            "Error revirtiendo pasar-a-consulta:",
            rollbackError
          );
        }
      }
